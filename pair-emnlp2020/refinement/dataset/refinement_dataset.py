...
class RefinementDataset(BaseDataset):
    ...
    def __init__(self, **kwargs):
        ...
        path = f'../data-fact/refinement/{self.domain}/{self.set_type}.jsonl'
        self.template = []
        self.load_raw_dataset(path=path)
        print("#"*20,"check template and tgt length: ")
        count = 0
        for i, (temp, sent_tgt) in tqdm(enumerate(zip(self.template, self.target))):
            #assert len(temp) == len(sent_tgt) - 1
            #print(len(temp), len(sent_tgt))
            if len(temp) != (len(sent_tgt) - 1):
                count += 1
                print(i)
                #print(len(temp), len(sent_tgt))
        print('{} example may cause problem'.format(count))
    ...
    def __getitem__(self, index):
        ...
        def generate_mask(strategy):
            ...
            if strategy == 'random':
                ...
                #print('#'*20, len(masked_draft), len(template_ids))
                non_kp_masked_draft = []
