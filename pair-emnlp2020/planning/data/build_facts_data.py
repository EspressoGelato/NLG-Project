...
from utils import (
    ...
    get_facts_topic_signatures,
    generate_template_for_bart,
)



def load_facts_rawdata(root_path, set_type:str):
    ...
    for file in sorted(os.listdir(set_path)):
        ...
        with open(path, 'r') as f:
            cur_obj = json.load(f)
            cur_id = cur_obj['docID']


        body_str = cur_obj['text'].strip('<sep><cls>')
        prompt = cur_obj['openfacts'][0]['text']

        ret_obj = dict(id=cur_id,
                        prompt=prompt,
                        tgt = body_str)
...
def run_corenlp(raw_data, root_path, set_type, save_to_disk=True):
    """Run StanfordCoreNLP pipeline (tokenize,ssplit,pos)

    If the results is already in the disk, load without running.
    """
    output_path = f'{root_path}/{set_type}.corenlp.jsonl'
...
def run_chunking(corenlp_results, root_path, set_type, save_to_disk=True):
    """Run regex based chunking for NP and VP.
    """
    print("run chunking")
    output_path = f'{root_path}/{set_type}.chunk.jsonl'
    if os.path.exists(output_path):
    ...
    for ln in tqdm(corenlp_results):
        ...
        if ln['proc'] == "Could not handle incoming annotation":
            print("#"*50)
            print('*'*10, ln['proc'], "*"*20)
            print(type(ln['proc']))
            num_invalid += 1
    ...
    print('chunking results loaded')
    print("number of invalid: ", num_invalid)
    return chunk_results



def generate_kp(raw_data, root_path, set_type, cache_intermediate):
    """Generate keyphrase from raw text, need to run StanfordCoreNLP first,
    then regex based chunking
    """
    output_path = f'{root_path}/{set_type}.kp.jsonl'

    corenlp_results = run_corenlp(raw_data, root_path, set_type, cache_intermediate)
    if os.path.exists(output_path):
        kp_data = [json.loads(ln) for ln in open(output_path)]
        print('kp data loaded')
        return kp_data, corenlp_results

    chunk_results = run_chunking(corenlp_results, root_path, set_type, cache_intermediate)
    topic_sig = get_facts_topic_signatures()
    kp_data = []
...
def create_planning_data(raw_data, kp_data, proc_data, root_path, set_type):
    fout = open(f'{root_path}/planning_{set_type}.jsonl', 'w')
    ...
    for ln_id, (proc, kp, raw) in tqdm(enumerate(zip(proc_data, kp_data, raw_data))):
        ...
        if proc['proc'] == "Could not handle incoming annotation":
            count += 1
            print("count: ", count)
            #continue
        ...
        if len(tgt_tokenized) >= 1024:
            print("#"*100, len(tgt_tokenized))

        if len(kp_set_tokenized) >= 1024:
            print("#"*100, len(kp_set_tokenized))

        if len(kp_tokenized) >= 1024:
            print("#"*100, len(kp_tokenized))

        ret_obj = dict(
                id=proc['id'],
                tgt=tgt_tokenized,
                original_kp=proc_kp,
                kp_tgt_offsets=kp_offsets,
                kp_set=kp_set_tokenized,
                kp_tgt_tokens=kp_tokenized,
...
def create_refinement_data(raw_data, kp_data, proc_data, root_path, set_type):
    ...
    fout = open(f'{root_path}/refinement_{set_type}.jsonl', 'w')
...
def main():
    ...
    parser.add_argument("--set_type", choices=['train', 'valid', 'test', 'toy'],
                        required=True)
    ...
    raw_data = load_facts_rawdata(root_path= root_path, set_type=args.set_type)
