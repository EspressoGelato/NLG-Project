...
from datareaders.MaskedPlotGenreDataset import MaskedPlotGenreDataset, GENRES_LIST
from datareaders.PlotFactsOnlyDataset import PlotFactsOnlyDataset
...
#ALL_MODELS = sum((tuple(conf.pretrained_config_archive_map.keys()) for conf in ( XLNetConfig, TransfoXLConfig)), ())
...
def main():
    ...
    def collate(examples: List[torch.Tensor]):
        ...
        seqs, facts, masks, genres, ids = zip(*examples)
        ...
        torch.stack(genres)
        return list(zip(pad_facts, pad_mapping, genres, pad_originals, ids))
    ...
    for batch in test_dataloader:
        batch_counter+=1
        with torch.no_grad():
            pad_facts, pad_mapping, genres, pad_originals, ids = zip(*batch)
            ...
            #print("tpad_facts:, ", tpad_facts, tpad_facts.shape)
            seqs, masks = get_full_seqs_and_masks(pos_outputs[0][0].squeeze(-1).long(), tpad_facts[0], tokenizer)
            #print(pos_outputs)
            seqs_tokens = tokenizer.convert_ids_to_tokens(seqs)
            seqs_sent = tokenizer.convert_tokens_to_string(seqs_tokens).strip('<sep><cls>')
            tokens_bart = bart_tokenizer.tokenize(seqs_sent)

            for i, token in enumerate(tokens_bart):
                if token == '<unk>':
                    tokens_bart[i] = None
            
            #print(ids[0].item())
            if ids[0].item() not in res:
                res[ids[0].item()] = tokens_bart
            ...
            time2 = time.time()
            print('time used for prediction: ', time2 - time1)

            from datareaders.PlotFactsOnlyDataset import GENRES_LIST
            ...
            '''
    with open('val_fact_pos_prediction.json', 'w') as f:    
